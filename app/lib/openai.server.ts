...
你刚刚说的话是：${originalText}（共${originalText.length}个字）

作为你的沟通小助手，我会帮你重新整理这句话，让它听起来更温暖，也更容易被对方接受。我会从四个角度来帮你，最后给出一个长度适中（${
      originalText.length
    }-${originalText.length * 3}字之间）的标准答案：
...

{
  ...
  "standard_response": "一句综合了观察、感受、需要、请求的完整标准答案。长度控制在原话的1-3倍之间，既完整表达又简洁有力，温暖直接"
}
...
5. 标准答案：把四个部分自然地串联成一句完整的话，就像你直接跟对方说话一样，要温暖、诚恳、容易理解。长度控制在原话的1-3倍之间，既要表达完整，又不能太啰嗦。这应该是你可以直接使用的完美表达。
